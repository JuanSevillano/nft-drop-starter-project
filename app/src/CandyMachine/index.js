...
const CandyMachine = ({ walletAddress }) => {
  ...
  const getProvider = () => {
    ...
    // Object with connection and user creds, so now can call solana programs
  ...
  const getCandyMachineState = async () => {
    ...
    // Get metadata about candy machine program deployed 
    const idl = await Program.fetchIdl(candyMachineProgram, provider);

    // create a program that can be call
    ...
    const goLiveDateTimeString = `${new Date(goLiveData * 1000).toLocaleDateString()}`;

    // Saving data to app state
    setMachineStats({
      itemsAvailable,
      itemsRedeemed,
      itemsRemaining,
